<template>
    <div v-if="!isLoading">
        <div v-if="restaurant.restaurantSelected.restaurant" class="w-full">
            <div class="py-[20px] flex justify-center">
                <div class="max-w-[1200px] px-[24px] lg:px-[40px] w-full">
                    <div class="w-full grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                        <div>
                            <img class="w-full h-full object-cover" :src="restaurant.restaurantSelected.restaurant.avatar" alt="" />
                        </div>
                        <div v-for="image in restaurant.restaurantSelected.restaurant.images" :key="image.image">
                    ...
                    <div class="w-full mt-[40px] px-4 text-[14px] font-normal">
                        <div class="xl:flex gap-4 mb-6">
                            <div class="xl:w-2/3 border-primaryColor6 pb-4 mb-[40px]">
                                <h2 class="text-[26px] font-semibold mb-[13px]">{{ restaurant.restaurantSelected.restaurant.name }}</h2>
                                <div class="mt-[40px]">
                                    ...
                                    <div class="border-b border-primaryColor6 py-[20px]">
                                        <h4>{{ restaurant.restaurantSelected.restaurant.phone }}</h4>
                                    </div>
                                    <div class="border-b border-primaryColor6 py-[20px]">
                                        <h4>{{ restaurant.restaurantSelected.restaurant.address }}</h4>
                                    </div>
                                </div>
                            </div>
                            <div v-if="restaurant.restaurantSelected.restaurant.allow_booking" class="xl:w-1/3">
                    ...
                    <div class="mt-[30px] flex items-center gap-3">
                        <div class="flex justify-center items-center">
                            <LikeButton @like="likeRestaurant()" @un-like="unLikeRestaurant()" :restaurant-id="restaurant.restaurantSelected.restaurant.id" />
                            {{ restaurant.restaurantSelected.restaurant.countLike }}
                        </div>
